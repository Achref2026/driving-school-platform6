...
async def accept_student_enrollment(
    ...
    try:
        ...
        # CRITICAL FIX: Update user role from "guest" to "student"
        ...
        if user_update_result.modified_count > 0:
            logger.info(f"Updated user {enrollment['student_id']} role to student after enrollment approval")
        else:
            logger.warning(f"Failed to update user {enrollment['student_id']} role to student")
        
        # Create sequential courses for the newly approved student
        await create_sequential_courses(enrollment_id)
...
# ====================================================================================
# ENHANCED MANAGER DASHBOARD APIs - Complete functionality except payments
# ====================================================================================

# Teacher Management APIs
@api_router.post("/manager/teachers")
async def add_teacher_to_school(
    teacher_data: TeacherCreate,
    current_user = Depends(get_current_user)
):
    """Add a new teacher to the manager's driving school"""
    try:
        if current_user["role"] != "manager":
            raise HTTPException(status_code=403, detail="Only managers can add teachers")
        ...
        teacher_user_doc = {
            ...
            "first_name": teacher_data.first_name or "",
            "last_name": teacher_data.last_name or "",
            "phone": teacher_data.phone or "",
            "address": teacher_data.address or "",
            "date_of_birth": teacher_data.date_of_birth or "",
            "gender": teacher_data.gender or "male",
        ...
        teacher_doc = {
            ...
            "rating": 0.0,
            "total_reviews": 0,
            "is_approved": True,  # Auto-approve for manager-added teachers
            "created_at": datetime.utcnow()
        }
        await db.teachers.insert_one(teacher_doc)
        
        # Return teacher with user details
        teacher_doc["user_details"] = {
            "first_name": teacher_data.first_name,
            "last_name": teacher_data.last_name,
            "email": teacher_data.email,
        ...
        return {
            "message": "Teacher added successfully",
            "teacher": serialize_doc(teacher_doc)
        }
...
@api_router.get("/manager/teachers")
async def get_school_teachers(current_user = Depends(get_current_user)):
    """Get all teachers for the manager's school"""
    try:
        if current_user["role"] != "manager":
        ...
        # Add user details for each teacher
        for teacher in teachers:
            user = await db.users.find_one({"id": teacher["user_id"]})
            if user:
                teacher["user_details"] = {
...
@api_router.delete("/manager/teachers/{teacher_id}")
async def remove_teacher(
    ...
    try:
        ...
        if not school:
            raise HTTPException(status_code=403, detail="Unauthorized to remove this teacher")
        ...
        await db.users.update_one(
            {"id": teacher["user_id"]},
            {"$set": {"is_active": False, "role": UserRole.GUEST}}
        )
        
        return {"message": "Teacher removed successfully"}
    
    except Exception as e:
        logger.error(f"Remove teacher error: {str(e)}")
        if isinstance(e, HTTPException):
            raise e
        raise HTTPException(status_code=500, detail="Failed to remove teacher")

# Session Management APIs
@api_router.post("/manager/sessions")
async def schedule_session(
    session_data: SessionCreate,
    current_user = Depends(get_current_user)
):
    ...
    try:
        if current_user["role"] != "manager":
            raise HTTPException(status_code=403, detail="Only managers can schedule sessions")
        ...
        if not school:
            raise HTTPException(status_code=403, detail="Unauthorized to schedule sessions for this school")
        ...
        if student:
            notification_doc = {
                ...
                "message": f"A new {course['course_type']} session has been scheduled for {datetime.fromisoformat(session_data.scheduled_at.replace('Z', '+00:00')).strftime('%B %d, %Y at %H:%M')}",
                "is_read": False,
                "metadata": {
...
@api_router.get("/manager/sessions")
async def get_school_sessions(current_user = Depends(get_current_user)):
    """Get all sessions for the manager's school"""
    try:
        if current_user["role"] != "manager":
            raise HTTPException(status_code=403, detail="Only managers can view school sessions")
        ...
        # Get all courses for these enrollments
        ...
        sessions_cursor = db.sessions.find({"course_id": {"$in": course_ids}}).sort("scheduled_at", -1)
        sessions = await sessions_cursor.to_list(length=None)
        
        # Add student and teacher names
        for session in sessions:
            student = await db.users.find_one({"id": session["student_id"]})
            teacher = await db.users.find_one({"id": session.get("teacher_id")})
            
            session["student_name"] = f"{student['first_name']} {student['last_name']}" if student else "Unknown Student"
            session["teacher_name"] = f"{teacher['first_name']} {teacher['last_name']}" if teacher else "Unknown Teacher"
...
@api_router.post("/manager/quizzes")
async def create_quiz(
    quiz_data: QuizCreate,
    current_user = Depends(get_current_user)
):
    """Create a new quiz for the school"""
    try:
        if current_user["role"] != "manager":
            raise HTTPException(status_code=403, detail="Only managers can create quizzes")
...
@api_router.get("/manager/quizzes")
async def get_school_quizzes(current_user = Depends(get_current_user)):
    """Get all quizzes for the manager's school"""
    try:
        if current_user["role"] != "manager":
            raise HTTPException(status_code=403, detail="Only managers can view school quizzes")
        ...
        # Get quizzes
        quizzes_cursor = db.quizzes.find({"school_id": school["id"]}).sort("created_at", -1)
        quizzes = await quizzes_cursor.to_list(length=None)
        
        return {"quizzes": serialize_doc(quizzes)}
...
@api_router.get("/manager/students/progress")
async def get_students_progress(current_user = Depends(get_current_user)):
    """Get progress overview for all students in the school"""
    try:
        if current_user["role"] != "manager":
            raise HTTPException(status_code=403, detail="Only managers can view student progress")
        ...
        for enrollment in enrollments:
            ...
            # Get quiz attempts
            quiz_attempts_cursor = db.quiz_attempts.find({"student_id": student["id"]})
            quiz_attempts = await quiz_attempts_cursor.to_list(length=None)
            avg_quiz_score = sum([qa["score"] for qa in quiz_attempts]) / len(quiz_attempts) if quiz_attempts else 0
            
            # Get sessions
            sessions_cursor = db.sessions.find({"student_id": student["id"]})
            sessions = await sessions_cursor.to_list(length=None)
...
async def get_manager_analytics_overview(current_user = Depends(get_current_user)):
    """Get comprehensive analytics overview for the manager's school"""
    try:
        if current_user["role"] != "manager":
        ...
        if enrollment_ids:
            ...
            if course_ids:
                sessions_cursor = db.sessions.find({"course_id": {"$in": course_ids}})
                sessions = await sessions_cursor.to_list(length=None)
            else:
                sessions = []
        else:
        ...
        # Calculate metrics
        total_enrollments = len(enrollments)
        approved_enrollments = len([e for e in enrollments if e["enrollment_status"] == "approved"])
        pending_enrollments = len([e for e in enrollments if e["enrollment_status"] == "pending_approval"])
        completed_sessions = len([s for s in sessions if s["status"] == "completed"])
        
        # Revenue estimate (simplified)
        estimated_revenue = approved_enrollments * school.get("price", 0)
        
        # Monthly enrollment trends (last 6 months)
        monthly_enrollments = {}
        for enrollment in enrollments:
            month_key = enrollment["created_at"].strftime("%Y-%m")
        ...
        analytics = {
            ...
            "overview_metrics": {
                ...
                "total_teachers": len(teachers),
                "active_teachers": len([t for t in teachers if t.get("is_approved", False)]),
                "total_sessions": len(sessions),
            ...
            "school_rating": school.get("rating", 0),
...
@api_router.get("/manager/school")
async def get_manager_school(current_user = Depends(get_current_user)):
    """Get the manager's driving school details"""
    try:
        if current_user["role"] != "manager":
            raise HTTPException(status_code=403, detail="Only managers can view school details")
        ...
        logger.error(f"Get manager school error: {str(e)}")
        if isinstance(e, HTTPException):
            raise e
        raise HTTPException(status_code=500, detail="Failed to retrieve school details")

@api_router.put("/manager/school")
async def update_manager_school(
    school_update: DrivingSchoolCreate,
    current_user = Depends(get_current_user)
):
    """Update the manager's driving school details"""
    try:
        if current_user["role"] != "manager":
            raise HTTPException(status_code=403, detail="Only managers can update school details")
        ...
        update_data = {
            ...
            "address": school_update.address,
            "state": school_update.state,
            "phone": school_update.phone,
            "email": school_update.email,
            "description": school_update.description,
            "price": school_update.price,
            "latitude": school_update.latitude,
            "longitude": school_update.longitude,
            "updated_at": datetime.utcnow()
        }
